...
def temp_config_file(tmp_path):
    config_data = {"command": ["python.exe"], "test_key": "test_value"}
...
def temp_args_json(tmp_path):
    args_data = {
        ...
        "params": {"param_output_type": "all", "target_files": ["/path/to/test.blend"]},
    }
...
def test_generate_html_content_with_links():
    links = [{"Link": "http://example.com", "Title": "Example", "Votes": 10}]
...
def test_generate_html_content_with_links_no_votes():
    links = [{"Link": "http://example.com", "Title": "Example"}]
